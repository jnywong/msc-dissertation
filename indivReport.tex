...
\usepackage{fancyhdr}
\usepackage[left=19mm,top=25mm,bottom=19mm, right = 22mm]{geometry}
\pagestyle{fancy}
...
%\titlelabel{\thetitle.\quad\hspace*{-3mm}}
...
%The axial and radial stresses in Equation \ref{eqn:mom_con} are determined by their relationship with the strain rate \cite{goldin1969breakup}, most generally described by
%\begin{align}
%\sigma_{ij} = \int^t_{-\infty} G(t - t')E_{ij}(t') dt'
%\end{align}
%where $G(t)$ is the elastic modulus and $E_{ij}$ is the strain rate tensor.
%
%The elastic modulus $G(t)$ is obtained experimentally via an oscillatory shear experiment, usually a Couette device. A homogeneous shear is imposed on the flow  of the form
%\begin{align*}
%\gamma(t) = \gamma_0 \sin (\omega t)
%\end{align*}
%where $\gamma_0$ is the amplitude of the shear and $\omega$ is the frequency. The shear rate is 
%\begin{align*}
%\dot{\gamma}(t) = \gamma_0 \omega \cos (\omega t).
%\end{align*}
%and therefore the stress in the flow is
%\begin{align*}
%\sigma(t) = \int^t_{-\infty} G(t-t') \gamma_0 \omega \cos (\omega t') \, dt'.
%\end{align*}
%A variable substitution $s = t-t'$ yields
%\begin{align*}
%\sigma(t) &= \gamma_0 \omega \int^t_{-\infty} G(s) \cos(\omega(t-s)) \, ds \\
%&= \gamma_0 \omega \int^t_{-\infty} G(s) \Re\left[\exp (i \omega (t-s)\right] \, ds \\
%& = \gamma_0 \omega \Re \left[ \exp(i \omega t) \int^\infty_0 G(s) \exp(-i \omega s) \, ds \right].
%\end{align*}
...
%G^* = G^* + i G''
%\end{align*}
%where $G'$ is called the storage modulus and $G''$ is the loss modulus. Therefore the expression for the stress becomes
%\begin{align}
...
%\end{align}
%It can be seen that storage modulus is in phase with the shear and describes the fluids ability to store elastic energy. The loss modulus is in phase with the shear rate and describes the dissipation of energy, in other words the viscosity. Therefore
%\begin{align}
%\mu'= \frac{G''}{\omega}
%\end{align}
...
%\begin{align*}
%G' = \frac{G \omega^2 \tau^2}{1 + \omega^2 \tau^2} \hspace{2cm} G'' = \mu_s \omega + \frac{G \omega \tau}{1+\omega^2 \tau^2}.
%\end{align*}
...
%\section{1D Numerical Simulation} \label{sec:1D}
%The slender jet equations in Section \ref{sec:slender} are non dimensionalised by choosing the initial jet radius $R_0$ as the characteristic length scale and the Rayleigh capillary time $t_R = \sqrt{\rho R_0^3/ \gamma}$ as the characteristic time scale, which gives
%\begin{align}
%\pd{h^2}{t} + \pd{}{z} (h^2v) &= 0 \label{eqn:1D_mass_con} \\
%\rho \left( \pd{}{t} (h^2v) + \pd{}{z} (h^2v^2) \right) &= \pd{}{z} \left(h^2 \left( K + 3 Oh \pd{v}{z}\right) \right), \label{eqn:1D_mom_con}
%\end{align}
%for the Ohnesorge number $Oh = \mu_s / \sqrt{\rho \gamma R_0}$ and the curvature term $K$ is defined as
%\begin{align*}
%K \equiv \frac{1}{h(1+h_z^2)^{1/2}} + \frac{h_{zz}}{(1 + h_z^2)^{3/2}}.
%\end{align*}
%For a non Newtonian fluid additional stress contributions according to the Oldroyd B constitutive model (see Section \ref{sec:oldroyd}) require solving in addition to the slender jet equations
%\begin{align*}
%\pd{\sigma_{zz}}{t} + \pd{}{z} (v \sigma_{zz}) &= 3 \sigma_{zz} \pd{v}{z} + 2 \frac{\mu_p}{\tau} \pd{v}{z} - \frac{\sigma_{zz}}{\tau} \nonumber \\
%\pd{\sigma_{rr}}{z} + \pd{}{z}(v \sigma_{zz}) &= - \frac{\mu_p}{\tau}\pd{v}{z} - \frac{\sigma_{rr}}{\tau}
%\end{align*}
%where $\tau$ is the relaxation time and $\mu_p = G\tau$ is the polymer contribution to the viscosity for elastic modulus $G$. The non dimensional form of these equations are
%\begin{align}
%\pd{\sigma_{zz}}{t} + \pd{}{z} (v \sigma_{zz}) &= 3 \sigma_{zz} \pd{v}{z} + \frac{2c}{De} \pd{v}{z} - \frac{\sigma_{zz}}{De} \nonumber \\
%\pd{\sigma_{rr}}{z} + \pd{}{z}(v \sigma_{zz}) &= - \frac{c}{De}\pd{v}{z} - \frac{\sigma_{rr}}{De}
%\label{eqn:1D_poly}
%\end{align}
%where the Deborah number $De$ is defined as
%\begin{align*}
%De = \frac{\tau}{T}
%\end{align*}
%and the polymer concentration is defined as
%\begin{align*}
%c = \frac{\mu_p}{\mu_s} = \frac{G \tau}{\mu_s}.
%\end{align*}
%
%The simulation of the thinning jet is evaluated in the frame of a moving Lagrangian element moving at jet velocity $V_j$. At time $t=0$, the jet is assumed to be at rest and a small perturbation of the form
%\begin{align*}
%h(z,0) = R_0 + h' \cos \left( \frac{2 \pi z}{\lambda} \right)
%\end{align*}
%is applied to the free surface height $h = h(z,t)$, where $\lambda$ is the length of a single wavelength. 
%
%\subsection{The Lax Wendroff Scheme}
%Equations \ref{eqn:1D_mass_con}, \ref{eqn:1D_mom_con} and \ref{eqn:1D_poly} in the case for non Newtonian fluids can be solved using the Lax Wendroff scheme \cite{press1992numerical}. In the Newtonian case the equations can be written in flux conservative form
%\begin{align*}
%\pd{a}{t} &= - \pd{F}{z} \\
%\pd{F}{t} &= - \pd{G}{z}
%\end{align*}
%where $a = h^2$, $F = h^2v$ and $G = h^2 \left( v^2 - K - 3 Oh \pd{v}{z} \right)$. This is solved on a uniformly spaced mesh with nodes $z_j$ at time steps $t_n$ given by
%\begin{align*}
%z_j &= z_0 + j dz, \hspace*{1cm} j = 0, 1, ..., J \\
%t_n &= t_0 + n dt, \hspace*{1cm} n = 0 , 1, ..., N
%\end{align*}
%with widths $dz$ and time step sizes $dt$. In terms of notation, $a^n_j$ is equal to $a(z_j, t_n)$ for a specific grid point and time step. 
%
%The Lax Wendroff scheme employs a forward difference in time and central differencing in space, which is second order accurate in space. However according to \cite{press1992numerical}, this is unstable and so a Lax averaging approximation is applied to $a^n_j$
%\begin{align*}
%a^n_j = \frac{1}{2} \left( a^n_{j+1} + a^n_{j-1} \right).
%\end{align*}
%
%Also the temporal accuracy is increased from first to second order by introducing a half time step $t_{n + 1/2}$. The Lax Wendroff scheme is therefore a two step method as follows:
%\begin{spacing}{0.9}
%\begin{enumerate}
%\item Intermediate values are calculated at half time steps $t_{n + 1/2}$ and half mesh nodes $z_{j+1/2}$
%\begin{align*}
%a^{n+1/2}_{j+1/2} = \frac{1}{2}(a^n_{j+1} + a^n_j) - \frac{dt}{2 dz} (F^n_{j+1} - F^n_j) + O(dt, dz^2),
%\end{align*}
%in which the intermediate values of $F^{n+1/2}_{j+1/2}$ can then be computed.
%\item The nodal values of $a^{n+1}_j$ are then evaluated using
%\begin{align*}
%a^{n+1}_j = a^n_j - \frac{dt}{dz} (F^{n+1/2}_{j+1/2} - F^{n+1/2}_{j-1/2}) + O(dt^2, dz^2)
%\end{align*}
%and the intermediate values can now be discarded.
%\end{enumerate}
%\end{spacing}
%
%In the non Newtonian case, Equations \ref{eqn:1D_poly} are also solved. The right hand side of these equations are known as the source terms $S$ which are calculated separately and added to the final solution
%\begin{align*}
%\pd{a}{t} = - \pd{F}{z} + S.
%\end{align*}
%Therefore the two step method is now as follows:
%\begin{spacing}{0.9}
%\begin{enumerate}
%\item The Lax averaging approximation is applied to the source terms to give the intermediate values
%\begin{align*}
%a^{n+1/2}_{j+1/2} = \frac{1}{2}(a^n_{j+1} + a^n_j) - \frac{dt}{2 dz} (F^n_{j+1} - F^n_j) + \frac{dt}{4}(S^n_{j+1} + S^n_j) + O(dt, dz^2)
%\end{align*}
%\item The updated values are given by
%\begin{align*}
%a^{n+1}_j = a^n_j - \frac{dt}{dz} (F^{n+1/2}_{j+1/2} - F^{n+1/2}_{j-1/2}) + \frac{dt}{2}(S^{n+1/2}_{j + 1/2} + S^{n+1/2}_{j-1/2}) + O(dt^2, dz^2).
%\end{align*}
%\end{enumerate}
%\end{spacing}
%
%\subsection{Numerical Stability}
%Fourier stability analysis is used to evaluate the stability of the Lax Wendroff scheme.

% CFL condition, diffusion time, r_min < dz or else struggle with higher Oh
...
\begin{figure}[t]
	\begin{center}
		\subfloat[Needle 1 - the orifice design needle]{\includegraphics[width = 0.25\textwidth]{img/old-nozzle.png}} \hspace{5cm}
		\subfloat[Needle 2 - the needle design needle]{\includegraphics[width = 0.25\textwidth]{img/new-nozzle.png}}
...
\begin{figure}[h]
	\begin{center}
		\subfloat[The dimensionless wavenumber against the dimensionless growth rate for EHEC 0.4 wt.\% using Nozzle 2 $\left(Oh = 0.07, We = 20.5 \right)$. The predicted growth rate derived from the linear theory is plotted as a solid black line and the observed growth rate is denoted by the blue squares.]{\includegraphics[scale=0.5]{img/sweep4.png}} \hspace{1cm}
		\subfloat[The normalised jet midfilament diameter $D(t)/D_0$ for EHEC wt.\% $\left(Oh = 0.07, We = 20.5 \right)$. The jet is perturbed at apparent critical frequency $f = 2,500 Hz$ corresponding to the most amplified growth rate, and a lower frequency $f = 2,000 Hz$ corresponding to a wavenumber lower than the critical.]{\includegraphics[scale=0.5]{img/sweep7.png}}
	\end{center}
...
In conclusion the effective operation of ROJER is achieved between the critical wavenumber and the margin of stability $k^* R_0 \leq k R_0 \leq 1$. It is required that this range must be adjusted for empirically when using Nozzle 2 due to nozzle exit effects.
\begin{figure}[h]
	\begin{center}
		\subfloat[]{\includegraphics[scale=0.25, trim = 0cm 0cm 0cm 2.1cm, clip = true]{img/less_critical.png}}
		\subfloat[]{\includegraphics[scale=0.25]{img/more_critical.png}}
	\end{center}
	\caption{(a) Wavenumbers below the most unstable wavenumber $kR_0 < k^* R_0$ lead to the formation of satellite drops which merge with the leading drop. (b) Wavenumbers above the most unstable wavenumber suppresses the appearance of satellite drops and a stable beads on a string morphology is seen.}
	\label{fig:satellite}
\end{figure}
...
\multicolumn{1}{|c|}{Water}                              & 0.70       & $0.90$     & 2,949     & 0.008 & 531  & 19   \\ \hline
\multicolumn{1}{|c|}{Water-Glycerol $60 \mhyphen 40 \%$} & 0.65       & $0.96$     & 2,762     & 0.056 & 91   & 26   \\ \hline
\end{tabularx}
